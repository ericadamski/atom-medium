...
export default class MediumUploadView {
  ...
  updateView(user, state) {
    this.user = user;
    const username = $('.medium-upload-info-text-name');
    username.html(user.username);

    const userImage = $('.medium-upload-info-user-image');
    userImage.css({
      background: `url(${user.imageUrl}) no-repeat center`,
      'background-size': 'contain',
    });

    const message = $('.medium-upload-info-text-state');
    message.html(state);
  }

  attachHandlers(p) {
    $('.medium-public-btn').on('click', () => p.public());
    $('.medium-draft-btn').on('click', () => p.draft());
  }
