...
import MediumUploadView from './atom-medium-view';
...
function preparePostTitle(editorTitle) {
  ...
  {
    atom.notifications
      .addError('atom-medium : Can only operate on .md file extentions.');
    return false;
  }
...
function getStateAsString(state) {
  switch (state) {
    case Client.WRITTING:
      return `Currently writting post  <span class='icon icon-dash'></span><span class='atom-medium-post-title'>${getPost().title}</span>`;
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'atom-medium:toggle': () => this.toggle(),
    }));
  },
