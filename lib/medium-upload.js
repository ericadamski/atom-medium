...
export default {
  ...
  toggle() {
    ...
    this.mediumUploadView.attachHandlers(this.publish, this.draft);
  ...
  publish() {
    _this.mediumUploadView.updateViewState(getStateAsString(Client.POSTING));
    let _this = this;
    ...
    this.client
      ...
      .then(post => {
          ...
          setTimeout(() => _thi.modalPanel.hide(), 2000);
      }, err => {
  ...
  draft() {
    _this.mediumUploadView.updateViewState(getStateAsString(Client.DRAFTING));
    let _this = this;
