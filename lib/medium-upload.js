...
function preparePostTitle(editorTitle) {
    ...
    if (basename === editorTitle) {
        atom.notifications.addError(
            'medium-upload : Can only operate on .md file extentions.'
        );
        return false;
    }
...
function getStateAsString(state) {
    switch (state) {
        case Client.WRITTING:
            return `Currently writting post  <span class='icon icon-dash'></span><span class='medium-upload-post-title'>${
                getPost().title
            }</span>`;
...
export default {
    ...
    publish() {
        ...
        let post = getPost();
        this.client.publish(post.title, post.content).then(
            post => {
                _this.mediumUploadView.updateViewState(
                    getStateAsString(Client.SUCCESS)
                );
                atom.notifications.addSuccess(
                    'Success! Your post is now Public.'
                );
                setTimeout(() => _this.modalPanel.hide(), 2000);
            },
            err => {
                _this.mediumUploadView.updateViewState(
                    getStateAsString(Client.FAILURE)
                );
                atom.notifications.addError(
                    'There was an error making your post public.',
                    {
                        detail:
                            typeof err === 'object'
                                ? JSON.stringify(err)
    ...
    draft() {
        this.mediumUploadView.updateViewState(
            getStateAsString(Client.DRAFTING)
        );
        let _this = this;

        let post = getPost();
        this.client.draft(path.basename(post.title, '.md'), post.content).then(
            post => {
                _this.mediumUploadView.updateViewState(
                    getStateAsString(Client.SUCCESS)
                );
                atom.notifications.addSuccess(
                    `Success! Your draft is waiting for you @ ${
                        _this.client.user.url
                    }.`
                );
                setTimeout(() => _this.modalPanel.hide(), 2000);
            },
            err => {
                ...
                atom.notifications.addError(
                    'There was an error drafting your post.',
                    {
                        detail:
                            typeof err === 'object'
                                ? JSON.stringify(err)
