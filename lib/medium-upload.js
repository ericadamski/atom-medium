...
function preparePostTitle(editorTitle) {
  const basename = path.basename(editorTitle, '.md');
...
export default {
  ...
  serialize() {
    return { mediumUploadViewState: this.mediumUploadView.serialize() };
  },
  ...
  toggle() {
    ...
    this.client.on('user', user => this.mediumUploadView
  ...
  publish() {
    ...
    this.client
      ...
      .then(() => {
        this.mediumUploadView.updateViewState(getStateAsString(Client.SUCCESS));
        atom.notifications
          .addSuccess('Success! Your post is now Public.');
        setTimeout(() => this.modalPanel.hide(), 2000);
      }, err => {
        this.mediumUploadView.updateViewState(getStateAsString(Client.FAILURE));
        atom.notifications
  ...
  draft() {
    ...
    this.client
      ...
      .then(() => {
        this.mediumUploadView.updateViewState(getStateAsString(Client.SUCCESS));
        atom.notifications
          .addSuccess(`Success! Your draft is waiting for you @ ${this.client.user.url}.`);
        setTimeout(() => this.modalPanel.hide(), 2000);
      }, err => {
        this.mediumUploadView.updateViewState(getStateAsString(Client.FAILURE));
        atom.notifications
